...
import { lawBadge } from '@/lib/derive';
import type { Law } from '@/types/neo';

/**
 * S3 헤더의 상태 배지. `D-500` · `미이행` · `보류` · `시행중` 중 하나다.
 * 위험도 배지와 출처 배지는 날짜와 무관해 페이지가 서버에서 그린다.
 *
 ...
 * 법령의 기한은 사용자가 누구든 같다. 그래서 직접 읽는다.
 *
...
export function HeaderBadge({ law }: { law: Law }) {
  ...
  if (!today) return <Skeleton width={52} height={22} />;

  const badge = lawBadge(law, today);
  return (
