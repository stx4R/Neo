 ...
 * 홈의 "지금 해야 할 일"로 보내는** 안내다. 새 상태를 만들지 않는다.
 *
 * 미완이 0건이면 CTA 자체를 렌더하지 않는다. 할 일이 없는데 할 일 버튼을
 * 띄워 두지 않는다. 판단이 이 파일 안에 있으므로 페이지 쪽 여백은
 * 액션 유무가 아니라 'CTA가 있을 수 있는가'로만 잡는다.
 */
export function OpenActionsBar({ law }: { law: Law }) {
  ...
  return (
    <BottomCTA
      label={
        <>
          미완 액션 <span className="tnum">{open}</span>건 확인하기
        </>
      }
      onPress={() => router.push('/')}
    />
