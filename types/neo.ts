...
export interface Law {
   ...
   *
   * **빈 배열은 '품목을 가리지 않는다'는 뜻이다** — 상품 라벨 시행령처럼
   * HS 코드와 무관하게 모든 제품에 걸리는 법령이 여기 해당한다.
   * "모른다"가 아니라 "구분이 없다"이므로 영향 제품은 전 제품이 된다.
   */
  hsPrefixes: string[];
  /**
...
export interface Action {
  ...
  dueDate: string | null;
  /**
   * 이 액션이 해당되는 품목. 공용 법령(여러 품목에 걸치는 법령)에서만 쓴다.
   * 같은 법령이라도 식품과 화장품이 할 일이 다르다 —
   * 라벨 시행령 하나에 "영양성분 베트남어 표기"와 "전성분 베트남어 표기"가 같이 붙는다.
   * 값이 없으면 그 법령이 걸리는 모든 품목에 해당한다.
   */
  itemCategories?: ItemCategoryId[];
}
